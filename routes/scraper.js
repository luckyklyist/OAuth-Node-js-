...
router.post('/', checkAuth, async (req, res) => {
    ...
    try {
        const { seriesName,
            ...
            castName,
            scrapeImg,
            charDetail } = await scaper_data(showName);

        console.log(seriesName);

        res.render("index", { scraped: true, seriesName, charName, castName, scrapeImg, charDetail, error: false, showName: showN, authenticated });
    }
    catch (err) {
        console.log(err),
